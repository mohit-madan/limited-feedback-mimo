...
The scalar parametrization methods have also been implemented in 802.11ac standards \cite{lou2013comparison}. Surprisingly, there have been no performance comparisons between these two body of works, hence it is not quantifiable from past work if there are performance gains/losses by operating over scalar parametrization instead of manifold. In this paper, we propose a joint time-frequency interpolation approach that builds upon the existing parametric quantization techniques. One of the key results of our work is that the proposed method actually outperforms an equivalent manifold based technique \cite{Gupt1905:Predictive}, at the same time being analytically more tractable.
...
separately. Typical natural wireless channels don't just have independent time-frequency correlations, but also posses some novel time-frequency cross correlations. \cite{Gupt1905:Predictive} framed an equivalent manifold based approach to utilize joint time-frequency correlations for predictive quantization and interpolation, and showed performance benefits compared to utilizing time-frequency correlations independently \cite{6891198,khaled2005quantized}.
Intuitively, \cite{Gupt1905:Predictive} observes the performance benefits since utilizing joint correlations not only exploits the existing independent correlations, but also the novel cross correlations in time-frequency arising from the underlying channel models.
...
Secondly, instead of comparing to similar algorithms in the chosen body of work (viz. scalar parametrization), as has been the case with past works, we instead compare our proposed algorithm with an equivalent manifold based approach, and show drastic performance benefits over the manifold approach.
...
one-time instance and only $\bf{\Phi}$ values for the next time instant in
...
In both the situations, the only-time based interpolation using manifold
has the worst performance as it is not utilizing the joint time $\&$ frequency 
correlations. 
\begin{figure}
