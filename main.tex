...
The use of multiple antennas at the transmitter and receiver (MIMO) can
significantly enhance the performance of wireless systems. 
A typical step to get this performance enhancements is estimation of channel
state information (CSI).
If transmitter also has the knowledge of CSI, it can allocate 
resources more efficiently, both to
improve data rates and to reduce BER~\cite{love2008overview}. 

To this end, it is necessary to efficiently feedback the CSI from the receiver 
to the transmitter. In wireless systems that use orthogonal
frequency division multiplexing (OFDM), the feedback needs to be
provided to the transmitter across all subcarriers and in each OFDM frame, 
which places a large feedback demand. However, since the CSI varies gradually in both
time and frequency, effective techniques to reduce feedback overheads
exist, using frequency domain interpolation and temporal prediction [CITE?]. In
particular, knowledge of the precoding matrices (that are typically
the orthonormal right singular vectors of CSI matrix) is
essential to achieve the link capacity, as well as to parallelize the
...
efficient CSI feedback by tracking a parametrized version of the precoding matrices.
...
An alternate approach is to parametrize the precoding matrices using
...
While the manifold based methods have been shown to be very effective for precoder feedback,
...
to obtain, especially (don't repeat especially again) for joint time-frequency predictive coding.
In addition to being analytically more tractable, the scalar parametrization methods have also been
implemented in 802.11ac standards \cite{porat2013improved}. Past work in utilizing
...
separately. In this paper, we propose a method for exploiting the joint
...
Most of the papers utilizing manifolds \cite{Gupt1905:Predictive,pitaval2013codebooks,6891198}  and scalar parametrization \cite{4114278,4556174} compare the performance to other manifold/scalar parametrization methods, but don't do the cross comparison to highlight the performance benefits of the chosen method.
...
% manifolds~\cite{Gupt1905:Predictive}. 
Simulations reveal that the
