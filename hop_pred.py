...
#VQ Based Initialisation
    ...
    # ang_list=np.load('Codebooks/temp/base_quant_cb.npy')
    ...
    for simulation_index in range(number_simulations):
        ...
        if(simulation_index>=2*past_vals):
            ...
            for pred_indice in range(feedback_mats):
                if((simulation_index%2==1) and (pred_indice==feedback_mats-1)):
                    ...
                    onlyt_predU=onlyT_pred_givens(onlyt_prev_list[0],onlyt_prev_list)
                    # onlyt_predU = onlyT_pred(onlyt_prev_list[0],onlyt_prev_list)
                    # x,y=qtisn(onlyt_predU,onlyt_rU,1.5,20,sHt_list,norm_fn,sk=0.0)
                ...
                # predU1,F,T=MMSE_tangent_predict(prev_list,time_multipliers,freq_multipliers,center_index,rU,t_mult,f_mult,past_vals,False)
                predU2,F,T=MMSE_angle_predict(prev_list,time_multipliers,freq_multipliers,center_index,rU,t_mult,f_mult,past_vals,False)
                # pdb.set_trace()
                # onlyt_predU=onlyT_pred(onlyt_prev_list[0],onlyt_prev_list)  
                onlyt_predU=onlyT_pred_givens(onlyt_prev_list[0],onlyt_prev_list)            
                # Quantize and store the values in qU array for post analysis
                # qtiz_err,qtiz_U=qtiz_errsn(predU,rU,1.5,20,sHt_list,"stiefCD",sk=0.0)
                qtiz_err,qtiz_U=qtisn(predU2,rU,1.5,20,sHt_list,"stiefCD",sk=0.0)

                onlyt_qtiz_err,onlyt_qtiz_U=qtisn(onlyt_predU,onlyt_rU,1.5,20,sHt_list,"stiefCD",sk=0.0)
                # onlyt_qtiz_err,onlyt_qtiz_U=qtisn_givens(onlyt_predU,onlyt_rU,1.5,20,ang_list,"stiefCD",sk=0.0)
        ...
        # Initialisation criteria
            ...
            for i in range(feedback_mats):
                ...
                # In odd instances communicate only 31/7 matrices (middle)
