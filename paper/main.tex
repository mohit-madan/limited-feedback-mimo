...
The use of multiple antennas at the transmitter and receiver (MIMO)
can significantly enhance the performance of wireless systems.  A
typical step to get this performance enhancements is estimation of
channel state information (CSI).  If transmitter also has the
knowledge of CSI, it can allocate resources more efficiently, both to
improve data rates and to reduce BER~\cite{love2008overview}.  To this
end, it is necessary to efficiently feed back CSI from the receiver to
the transmitter. In wireless systems that use orthogonal frequency
division multiplexing (OFDM), the feedback needs to be provided to the
transmitter across all subcarriers and in each OFDM frame, which
places a large feedback demand. However, since the CSI varies
gradually in both time and frequency, effective techniques to reduce
feedback overheads exist, using frequency domain interpolation and
temporal prediction [CITE?]. In particular, knowledge of the precoding
matrices (that are typically the orthonormal right singular vectors of
CSI matrix) is essential to achieve the link capacity, as well as to
parallelize the channel. In this paper, we focus on the problem of
tracking the precoders of a MIMO-OFDM system across time and frequency
for efficient CSI feedback by tracking a parametrized version of the
precoding matrices.
...
scalar parameters. The space of unitary matrices can be parameterized into
...
interpolation may be difficult to obtain, especially (don't repeat
especially again) for joint time-frequency predictive coding.  In
addition to being analytically more tractable, the scalar
parametrization methods have also been implemented in 802.11ac
standards \cite{porat2013improved}. Past work in utilizing scalar
...
on the transmitter using CSI feedback.  Typical approaches to
predictive quantization and interpolation do not exploit the joint
...
$\Theta_{i,t}$ has length $N_{R}(2N_{T} - N_{R}+1)/2$.~\cite{4114278}
\begin{figure}
...
can be reduced by half if we transmit only $\Theta$(collection of $\theta_{i}s$) values for
one-time instance and only $\Phi$(collection of $\phi_{i}s$) values for the next time instant in
...
\begin{figure}
\includegraphics[width=0.5\textwidth]{images/1024timebased}
...
\begin{figure}
\includegraphics[width=0.5\textwidth]{images/64timebased}
