...
decomposed as TODO
\begin{equation}
\textbf{V} = \Bigg[\prod_{k=1}^{n} \bD_{k} \big( \phi_{k,k},\ldots , \phi_{k,n} \big) \: \Bigg\{ \prod_{l=1}^{l=t-k} \bG_{t-l,t-l+1} \big( \theta_{k,l}\big) \Bigg\} \Bigg] \: \tilde{\bI}
\end{equation}
where $D_{k}$ is the diagonal matrix given as $D_{k}\big(\phi_{k,k},
\ldots, \phi_{k,t} \big)$ =  $\mbox{diag}\big( {\bf 1}_{k-1},
e^{j\phi_{k,k}},\ldots, e^{j\phi_{k,t}}  \big)$, ${\bf 1}_{k-1}$ is
represents  $k-1$ ones, and the $N_R \times N_T$ matrix $\tilde{\bI}$
= $\big[\bI_{n}, {\bf 0}_{n,t-n}\big]^{T}$, and
\begin{equation}
...
\end{align*}
Here, $\phi_{k}$s are referred to as phases and $\theta_{l}$s as
rotation angles. $\phi_k \in (-\pi, + \pi]$ is uniformly distributed
...
\begin{figure}
\includegraphics[width=0.45\textwidth]{images/new-adpm.pdf}
...
quantized at every $s$-th subcarriers, for even time instances, where,
$s$ is the gap between two quantized subcarrier indices. For odd time
instances, subcarriers at the position $sk$ for k = 0,1,...,
$\frac{N-1}{s}$ with t = ${\frac{s}{2}}$. TODO
...
{ADPM Design}: The decrement rate - \bm is 2.4 times the \bM - increment rate in the Equation~\ref{delta_eqn} . This ratio was decided after trying with different ratios. This implies that if quantied values lose track of the actual values then after changing delta quickly they start following it again.
