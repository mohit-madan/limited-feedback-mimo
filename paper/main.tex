...
independent parameters as given in Equation~\ref{eq:decompose}. To
improve the precoder estimates and interpolation accuracy, we ``walk
back'' to improve the past estimates by interpolating using current
estimates (although the current estimates only use data
causally). Thus, the approach here is able to exploit the time and
frequency correlation jointly to enhance performance.
