...
\begin{figure}
\begin{center}
\includegraphics[width=0.7\columnwidth]{images/new-adpm1.pdf}
...
% accuracy. 
In Fig.~\ref{fig:adpm-fig}, each parameter is interpolated
using the neighbouring subcarriers (red arrows) using bilinear interpolation
...
Equation~\ref{eq:decompose}. Inorder to improve the precoder estimates we walkback to improve the past estimates (black arrows) by interpolating
upon the current estimates. Thus, the approach here is able to exploit the time and frequency correlation jointly to enhance performance.
