...
flexibility and ability to use the wideband channels effectively. To
utilize such channels effectively, the transmitter needs to have the
channel state information (CSI), since this permits channel
parallelization for low-complexity decoders as well as the use of
capacity achieving transmission
...
feeding back CSI from the receiver to the transmitter needs large
feedback overheads, since a precoder needs to be fed back for each
OFDM subcarrier within each coherence interval. Past approaches have
typically used manifold based quantization techniques that exploit
temporal and frequency correlation to reduce feedback
overheads. Wireless standards have largely employed parametric
quantization that uses Givens rotations based
quantization~\cite{lou2013comparison,5671092}, though these do not
involve any refinement of the estimates using channel correlations. In
this paper, we propose a joint time-frequency interpolation approach
that builds upon the parametric quantization techniques used by
standards that manages to outperform typical manifold based
...
\begin{figure}
\begin{center}
\includegraphics[width=0.37\textwidth]{images/qerror.pdf}
...
\begin{figure}
\begin{center}
\includegraphics[width=0.37\textwidth]{images/1024final_withtime}
...
proposed approach outperforms the manifold based approach
significantly, and the gap from the optimal precoder performance
narrows further when the speed reduces.
...
\begin{figure}
\begin{center}
\includegraphics[width=0.37\textwidth]{images/64final_withtime}
