...
both the ITU Vehicular-A channel and the COST channel power delay
profiles~\cite{recommendation1997guidelines,molisch2006cost259,cost1989cost}. The
BER achieved by the proposed predictive quantization based precoder is
compared against the ideal precoder for 100 channel evolutions in each
run and averaged. The normalised Doppler is assumed to be
$f_dT_s = 3.5\times10^{-4}$, with symbol time period
$T_s = 5\times10^{-8} s$. For both the Vehicular and Pedestrian
channel profiles, we consider $N_T=4$ and $N_R=2$. Therefore, the
total number of parameters that determine the precoder is
$N_{R}(2N_{T} - N_{R}) = 12$, with five $\theta_l$s and and seven
$\phi_i$s. For the fast fading channels, we assume that the number of
subcarriers is 1024, while for the slower varying channel profiles, we
take 64 subcarriers. We assume that the spacing between fed back
subcarrier indices $p$ to be $33$ and $9$ for the fast and slow
channel profiles respectively (i.e. for the slower channels, the
subcarriers indices fed back would be $0, 33, 66, \ldots 1023$ and for
the Pedestrian, $0, 9, 18, \ldots 63$).
