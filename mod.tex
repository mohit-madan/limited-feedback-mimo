...
%DIF DEL main_prev.tex   Mon Dec 16 18:34:52 2019
%DIF ADD main.tex        Mon Dec 16 18:30:58 2019
\def\b0{{\bf 0}}
\def\ba{{\bf a}}
\def\bb{{\bf b}}
\def\bc{{\bf c}}
...
The use of multiple antennas at the transmitter and receiver \DIFaddbegin \DIFadd{(MIMO) }\DIFaddend can
significantly enhance the performance of wireless systems. 
\DIFdelbegin \DIFdel{Moreover,
in these multiple-input multiple-output (MIMO)wireless systems, the
transmitter }\DIFdelend \DIFaddbegin \DIFadd{A typical step to get this performance enhancements is estimation of channel
state information (CSI).
If transmitter also has the knowledge of CSI, it }\DIFaddend can allocate 
resources more efficiently\DIFdelbegin \DIFdel{if the channel
state information (CSI) is made available at the transmitter}\DIFdelend , both to
improve data rates and to reduce BER~\cite{love2008overview}. 
\DIFaddbegin 

\DIFaddend To this end, it is necessary to efficiently feedback the CSI from the receiver 
to the transmitter. In wireless systems that use orthogonal
frequency division multiplexing (OFDM), the feedback needs to be
provided to the transmitter across all subcarriers \DIFdelbegin \DIFdel{, and this }\DIFdelend \DIFaddbegin \DIFadd{and in each OFDM frame, 
which }\DIFaddend places a large feedback demand. However, since the CSI varies gradually in both
time and frequency, effective techniques to reduce feedback overheads
exist, using frequency domain interpolation \DIFdelbegin \DIFdel{temporal prediction}\DIFdelend \DIFaddbegin \DIFadd{and temporal prediction }[\DIFadd{CITE?}]\DIFaddend . In
particular, knowledge of the precoding matrices (that are typically
the orthonormal right singular vectors of \DIFdelbegin \DIFdel{the channel }\DIFdelend \DIFaddbegin \DIFadd{CSI }\DIFaddend matrix) is
essential to achieve \DIFaddbegin \DIFadd{the }\DIFaddend link capacity, as well as to parallelize the
...
efficient CSI feedback by tracking a parametrized version of the \DIFaddbegin \DIFadd{precoding }\DIFaddend matrices.
...
quantization and interpolation across time and frequency. 
\DIFdelbegin \DIFdel{While these
}\DIFdelend \DIFaddbegin \DIFadd{An alternate approach is to parametrize the precoding matrices using
...
\mbox{%DIFAUXCMD
\cite{4114278,4556174} }%DIFAUXCMD
}[\DIFadd{?}]\DIFadd{.   
}

\DIFadd{While the manifold based }\DIFaddend methods have been shown to be very effective for precoder feedback,
...
to obtain, especially \DIFaddbegin \DIFadd{(don't repeat especially again) }\DIFaddend for joint time-frequency predictive coding.
\DIFdelbegin \DIFdel{An
...
\mbox{%DIFAUXCMD
\cite{4114278,4556174}}%DIFAUXCMD
. These approaches have been combined with
}\DIFdelend \DIFaddbegin \DIFadd{In addition to being analytically more tractable, the scalar parametrization methods have also been
implemented in 802.11ac standards \mbox{%DIFAUXCMD
\cite{porat2013improved}}%DIFAUXCMD
. Past work in utilizing
scalar parametrization for precoder feedback have utilized
}\DIFaddend adaptive delta modulation effectively track precoding matrices both in
the time~\cite{4114278} and frequency domains~\cite{4556174}, albeit
separately. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend In this paper, we propose a method for exploiting the joint
time-frequency correlation of the precoder's scalar parameters on the
transmitter using CSI feedback. 
Typical approaches to predictive
...
\DIFdelbegin \DIFdel{Further, the independence of the scalar parameters reduces the problem to one that involves
interpolation of separate scalar valued functions, as opposed to operations over tangent spaces in
manifolds~\mbox{%DIFAUXCMD
\cite{Gupt1905:Predictive}}%DIFAUXCMD
.
}\DIFdelend \DIFaddbegin 

\DIFadd{Most of the papers utilizing manifolds \mbox{%DIFAUXCMD
\cite{Gupt1905:Predictive,pitaval2013codebooks,6891198}  }%DIFAUXCMD
and scalar parametrization \mbox{%DIFAUXCMD
\cite{4114278,4556174} }%DIFAUXCMD
compare the performance to other manifold/scalar parametrization methods, but don't do the cross comparison to highlight the performance benefits of the chosen method.
In addition to a novel joint time-frequency feedback scheme, we compare also the results of our
algorithm with an equivalent manifold based scheme, and illustrate the performance
benefits entailed by utilizing the scalar parametrization method. 
%DIF >   Further, the independence of
%DIF >  the scalar parameters reduces the problem to one that involves
%DIF >  interpolation of separate scalar valued functions, as opposed to
%DIF >  operations over tangent spaces in
%DIF >  manifolds~\cite{Gupt1905:Predictive}. 
}\DIFaddend Simulations reveal that the
...
\DIFdelbegin \DIFdel{The rest of this paper is organized as follows: Section~\ref{section2}
...
}\DIFdelend %DIF >  The rest of this paper is organized as follows: Section~\ref{section2}
%DIF >  describes the system model and the precoder feedback and interpolation
%DIF >  techniques. Section~\ref{section3} compares the BER and achievable
%DIF >  rates for our proposed methods with other approaches. Finally,
%DIF >  Section~\ref{section4} concludes with discussions on future work.
