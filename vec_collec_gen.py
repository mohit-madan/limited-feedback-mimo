    ...
    for time_index in range(2*past_vals+1):
        ...
        if(time_index==2*past_vals):
            ...
            for pred_indice in range(feedback_mats):     
                ...
                test_index=time_index%2*((feedback_mats+1)//2)+pred_indice*(feedback_mats+1)
                ...
                predU,F,T=MMSE_angle_predict(prev_list,time_multipliers,freq_multipliers,center_index,rU,t_mult,f_mult,past_vals,False)
                # pdb.set_trace()
                rT,vecrT=sH_lift(predU,rU,True)
                # vecrT=angle_diff(predU,rU)
                # pdb.set_trace()
                # Store in vec_collec
        ...
        # Initialisation criteria
            ...
            for i in range(feedback_mats):
                if(i==feedback_mats-1 and time_index%2==1):
                    continue
                else:    
                    oU=U_list[(feedback_mats+1)*i+time_index%2*((feedback_mats+1)//2)]
