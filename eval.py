...
def main():
    ...
    for chan_index in range(num_chan):
        ...
        tH_allH=np.load('Precoders_generated/Pedestrian8s/'+str(fdts)+'/th_allH_'+str(chan_index+chan_offset)+'.npy')
        tH_allU=np.load('Precoders_generated/Pedestrian8s/'+str(fdts)+'/th_allU_'+str(chan_index+chan_offset)+'.npy')
        tHS=np.load('Precoders_generated/Pedestrian8s/'+str(fdts)+'/thS_'+str(chan_index+chan_offset)+'.npy')
        ...
        onlyt_allU=np.load('Precoders_generated/Pedestrian8s/'+str(fdts)+'/allU_'+str(chan_index+chan_offset)+'.npy')
        interpS=np.load('Precoders_generated/Pedestrian8s/'+str(fdts)+'/interpS_'+str(chan_index+chan_offset)+'.npy')
...
if __name__ == '__main__':      
    ...
    # pdb.set_trace()
plt.yscale("log")
plt.plot(Eb_N0_dB,otBER_QPSK, marker='o')
plt.ylabel("BER")
plt.xlabel("Eb/N0(dB)")
plt.show()
x=np.arange(0,35,5)
plt.plot(x,avg_otcap, marker='o')
